...
var Validate = {
   ...
   *							requiresOneOf {Array} 			- an array of the string ids of the elements to validate
   *													  (DEFAULT: [])
   *							dependantFields {Array} 			- an array of the string ids of the elements that this element depends on.
   *                                                                                                      This field is only required if all the elements in this array has values.
   *													  (DEFAULT: [])
   */
  Presence: function(value, paramsObj){
    ...
    if(params.requiresOneOf.length > 0){
        var value_found = false;
        params.requiresOneOf.each( function(element){
            if (!$(element)) throw new Error("LiveValidation::initialize - No element with reference or id of '" + element + "' exists!");

            if($(element).value !== '' && $(element).value !== null && $(element).value !== undefined){
                value_found = true;
            }
        });
      if(!value_found) Validate.fail(params.failureMessage);
    }
    else if(params.dependantFields.length > 0){
        var values_found = true;
        params.dependantFields.each( function(element){
            if (!$(element)) throw new Error("LiveValidation::initialize - No element with reference or id of '" + element + "' exists!");
  ...
  /**
   *    THIS METHOD IS NOT PART OF THE LIVE VALIDATION LIBRARY.
   *    ADDED BY KEVIN TYLL AS A CONVENIENCE.
   *    
   *	validates that the field contains a valid date format suitable for mysql
   *
   *	@var value {mixed} - value to be checked
   *	@var paramsObj {Object} - parameters for this particular validation, see below for details
   *
  ...
  Date: function(value, paramsObj){
    ...
    Validate.Format(value, { failureMessage: params.failureMessage, pattern: /^\d{4}(\-)\d{2}\1\d{2}$/ } );
    return true;
  },
