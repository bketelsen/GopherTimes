...
function uploadSuccess(fileObj, server_data) {
	try {
		DisplayMessage(server_data);

		var progress = new FileProgress(fileObj,  this.customSettings.upload_target);

		progress.SetStatus("File being validated...");
		progress.ToggleCancel(false);
...
function DisplayMessage(src) {
	//var new_para = document.createElement("div");
	//new_para.style.margin = "0px";
	//document.getElementById("message").appendChild(new_para);
	//new_para.onload = function () { FadeIn(new_img, 0); };
	//new_para.innerHTML = src;
    //new_para.update(src);
    //new_para.highlight();
    $('message').innerHTML = src;
    $('message').highlight();
}
